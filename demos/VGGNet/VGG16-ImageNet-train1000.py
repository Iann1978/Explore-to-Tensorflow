...
print (model.summary())
model.save('checkpoints/vgg16-train1000/model1.h5')

# Compile
model.compile(loss='categorical_crossentropy',
              optimizer = SGD(lr=2e-3, decay=5e-4, momentum=0.9, nesterov=True),
              metrics=['accuracy'])


# Callback for ModelCheckpoint
checkpoint_path = "checkpoints/vgg16-train1000/checkpoint-weights_only-epoch{epoch:04d}-val_acc{accuracy:.4f}.ckpt"
checkpoint_dir = os.path.dirname(checkpoint_path)
print(checkpoint_dir)
cp_callback=ModelCheckpoint(checkpoint_path, verbose=1, save_weights_only=True,period=1)
# Callback for TensorBoard
tb_callback = TensorBoard(log_dir='logs\\vgg16-train1000\\')
...
# Train
model.fit_generator(generator=train_generator,steps_per_epoch=1281167/batch_size,epochs=50
